...
def main():
    args = parse_args()
    # Load dataset - handle both file and folder inputs
    if os.path.isdir(args.data):
        ...
        df = pd.concat(
            [pd.read_parquet(os.path.join(args.data, f)) for f in files],
            ignore_index=True,
        )
    else:
    ...
    train_df.to_parquet(os.path.join(args.train_out,
    "data.parquet"))
    val_df.to_parquet(os.path.join(args.val_out,
    "data.parquet"))
    test_df.to_parquet(os.path.join(args.test_out,
    "data.parquet"))
